...
h += '.sidebar{width:260px;background:#fff;border-right:1px solid #e5e7eb;padding:24px 0;position:fixed;top:0;left:0;height:100vh;overflow-y:auto;z-index:100;transition:transform .3s}\n'
h += '.sidebar.collapsed{transform:translateX(-260px)}\n'
...
h += '.sidebar_header p{font-size:12px;color:#6b7280}\n'
...
h += '@media(max-width:768px){.sidebar{transform:translateX(-260px)}.sidebar-toggle{left:0!important}.main-content{margin-left:0!important}}\n'
h += '</style>\n</head>\n<body>\n'
h += '<button class="sidebar-toggle" onclick="toggleSidebar()">☰</button>\n'
h += '<div class="sidebar" id="sidebar">\n<div class="sidebar-header">\n<h1>\U0001F525 Hellvape Trend Radar</h1>\n<p>Plugin Architecture v2.0  ' + now + '</p>\n</div>\n'
...
h += '<label style="white-space:nowrap;font-size:13px"><input type="checkbox" id="toggleFilter" checked onchange="toggleGeneric()">仅显示时效热点</label>\n</div>\n'
...
h += 'function toggleGeneric(){var show=document.getElementById("toggleFilter").checked,cards=document.querySelectorAll(".trend-card");var bl=[' + json.dumps(blacklist) + '];cards.forEach(function(card){var t=card.querySelector(".trend-title");if(t&&bl.some(function(b){return t.textContent.toLowerCase().includes(b)})){card.style.display="none"}else{card.style.display=""}})}\n'
...
h += 'document.querySelectorAll(".sidebar-item").forEach(function(item){item.addEventListener("click",function(){var p=item.getAttribute("data-platform");var target=document.querySelector("[data-platform=\\"" + p + "\\"]");if(target){target.click()}})});\n'
h += '</script>\n</body>\n</html>'
